...
function OneReview() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response1 = await fetch(
          "http://localhost:3001/api/review/contents"
        );
        const result1 = await response1.json();

        const response2 = await fetch(
          "http://localhost:3001/api/review/customer_names"
        );
        const result2 = await response2.json();

        const response3 = await fetch(
          "http://localhost:3001/api/review/rating"
        );
  ...
  if (
    reviewContent.length === reviewCustomer.length &&
    reviewRate.length === reviewCustomer.length
  ) {
          ...
          <div className="customer-pic-box">
            ...
            <div className="name-star-box">
              <div className="customer-name">
                {reviewCustomer[index].FirstName +
                  " " +
                  reviewCustomer[index].LastName}
              </div>
              <div className="review-star">
                {" "}
                <RatingStars rate={reviewRate[index].Rating}></RatingStars>
              </div>
            </div>
          </div>
